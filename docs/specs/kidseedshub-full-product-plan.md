...
- [x] 13. Health and special notes - Phase 2. Status: Verified. Verify: Admin-authenticated smoke updated `Smoke Revenue Flow Updated` with `healthNote: Feature 13 smoke - di ung dau phong`; `GET /api/classes/today` returned the note for `Robotics Demo - Hôm nay`; `GET /api/weekly-assessments/classroom?classId=cmp3bby2y001dpruxyn432qag` returned the same `healthNote`; Playwright verified `/classes` renders `Lưu ý sức khỏe` and `/assessments` renders the note in both the student rail and selected workspace header.
...
- [x] 24. Class skill comparison - Phase 2. Status: Verified. Verify: `/api/weekly-assessments/classroom?classId=cmp3bby2y001dpruxyn432qag` returned 8 `skillComparison` rows for `Robotics Demo - Hôm nay`, including observed skill averages such as `Sáng tạo` at `5/5`; Playwright verified `/assessments` renders `So sánh kỹ năng lớp` with the top skill row visible for the selected class/week.
...
- [x] 29. Debt warning - Phase 2. Status: Verified. Verify: `GET /api/dashboard/alerts` returned `debtWarnings` for 2 active enrollments with 0 remaining sessions and no newer receipt; `/dashboard` renders the `Cần thu học phí` panel with those students.
- [x] 30. Refund management - Phase 2. Status: Verified. Verify: refund smoke created `ABSENT_EXCUSED` attendance `cmpw0faly0009prh5hq5xr7ge`, refunded entitlement `cmpw0famk000bprh5598ear43` with reason `Refund smoke reason Feature 30`, generated linked expense `PC-2026-004`, and `GET /api/finance/summary?month=2026-06` returned `refundExpense: 1234` and `netRevenue: -1234`; `/finance` renders separate gross/refund/net revenue cards and labels the refund expense with the student.
...
- [x] 37. Schedule and notifications - Phase 2. Status: Verified. Verify: Parent demo user can see Pham Tuong Vy with 6 upcoming sessions and 5 relevant notices (SCHEDULE, WALLET, MAKEUP) in `/api/parent/portal`; Playwright verified `/parent` renders `Thong bao lien quan` and `Lich hoc sap toi`; admin-authenticated access to the parent portal API returns 403 FORBIDDEN.
