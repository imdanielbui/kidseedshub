...
- Rule 8C - SOLID/Clean Architecture By Default: new code must keep UI, application/business logic, data access, and infrastructure responsibilities separate. Route handlers should stay thin; React components should not own durable business rules; database access should stay behind backend/application helpers when logic is non-trivial. Add abstractions only when they improve testability, reuse, replacement, or reduce real coupling.
...
- Keep `AGENTS.md` as the global safety and execution entrypoint.
- Read `docs/ai-context/agent-routing.md` when a task needs explicit agent role behavior, handoff expectations, or role-specific output format.
- For ordinary coding/debugging/refactor work, follow the relevant role behavior without re-reading the full role catalog unless the task is ambiguous.
...
This project is indexed by GitNexus as **kidseedshub** (4628 symbols, 8733 relationships, 266 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
