import { PenLine, Plus, UsersRound } from "lucide-react"
...
import { formatMoneyInput, type ExpenseFormState, type OtherIncomeReceiptFormState } from "./finance-utils"
...
export function OtherIncomeReceiptDialog({ form, isSubmitting, onClose, onSubmit, setForm, students }: { form: OtherIncomeReceiptFormState; isSubmitting: boolean; onClose: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void; setForm: Dispatch<SetStateAction<OtherIncomeReceiptFormState>>; students: StudentListItem[] }) {
  const parents = useMemo(() => Array.from(new Map(students.filter((student) => student.parentName && student.parentPhone).map((student) => [student.parentPhone, { name: student.parentName, phone: student.parentPhone }])).values()).sort((left, right) => left.name.localeCompare(right.name, "vi")), [students])
  ...
  return (
    <DialogFormShell title="Tạo phiếu thu khác" eyebrow="Other income" description="Ghi nhận khoản thu không thuộc học phí. Phiếu này không làm thay đổi khóa học, số buổi hoặc ví học viên." onClose={onClose} onSubmit={onSubmit} size="lg" footer={<div className="flex flex-col-reverse gap-2 sm:flex-row sm:items-center sm:justify-between"><button type="button" className="neu-list-item rounded-2xl px-4 py-3 text-sm font-semibold text-stone-600 hover:text-brand-red" onClick={onClose}>Hủy</button><button type="submit" disabled={isSubmitting} className="glass-button-primary inline-flex items-center justify-center gap-2 rounded-2xl px-4 py-3 text-sm font-semibold disabled:cursor-not-allowed disabled:opacity-60"><Plus className="h-4 w-4" />{isSubmitting ? "Đang lưu" : "Lưu phiếu thu"}</button></div>}>
      <div className="grid gap-3 md:grid-cols-2">
        ...
        <label><span className="text-sm font-medium text-stone-600">Số tiền</span><input className="neu-pressed mt-2 w-full rounded-2xl bg-transparent px-4 py-3 text-sm text-brand-ink outline-none" inputMode="numeric" value={form.amount} onChange={(event) => setForm((current) => ({ ...current, amount: formatMoneyInput(event.target.value) }))} placeholder="100.000" required /></label>
        <fieldset className="md:col-span-2">
          <legend className="text-sm font-medium text-stone-600">Người nộp tiền</legend>
          <div className="mt-2 flex flex-wrap justify-end">
            <div className="inline-flex rounded-xl border border-brand-red/10 bg-white/50 p-1">
              <button type="button" aria-pressed={form.payerMode === "PARENT"} className={`inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-semibold transition ${form.payerMode === "PARENT" ? "bg-brand-red text-white shadow-sm" : "text-stone-500 hover:text-brand-red"}`} onClick={() => setForm((current) => ({ ...current, payerMode: "PARENT", payerParentPhone: "", payerName: "", payerPhone: "" }))}><UsersRound className="h-3.5 w-3.5" />Phụ huynh</button>
              <button type="button" aria-pressed={form.payerMode === "MANUAL"} className={`inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-semibold transition ${form.payerMode === "MANUAL" ? "bg-brand-red text-white shadow-sm" : "text-stone-500 hover:text-brand-red"}`} onClick={() => setForm((current) => ({ ...current, payerMode: "MANUAL", payerParentPhone: "", payerName: "", payerPhone: "" }))}><PenLine className="h-3.5 w-3.5" />Người khác</button>
            </div>
          </div>
          {form.payerMode === "PARENT" ? (
            <label className="mt-2 block"><span className="sr-only">Chọn phụ huynh</span><select className="neu-pressed w-full rounded-2xl bg-transparent px-4 py-3 text-sm font-medium text-brand-ink outline-none" value={form.payerParentPhone} onChange={(event) => { const parent = parents.find((item) => item.phone === event.target.value); setForm((current) => ({ ...current, payerParentPhone: event.target.value, payerName: parent?.name ?? "", payerPhone: parent?.phone ?? "" })) }} required><option value="">Chọn phụ huynh để tự điền thông tin</option>{parents.map((parent) => <option key={parent.phone} value={parent.phone}>{parent.name} - {parent.phone}</option>)}</select></label>
          ) : (
            <div className="mt-2 grid gap-3 md:grid-cols-2"><FinanceInput label="Họ tên" value={form.payerName} onChange={(value) => setForm((current) => ({ ...current, payerName: value }))} required /><FinanceInput label="Số điện thoại" value={form.payerPhone} onChange={(value) => setForm((current) => ({ ...current, payerPhone: value }))} /></div>
          )}
        </fieldset>
