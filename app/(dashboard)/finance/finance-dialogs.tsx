  ...
  return (
    <DialogFormShell title="Tạo phiếu thu khác" eyebrow="Other income" description="Ghi nhận khoản thu không thuộc học phí. Phiếu này không làm thay đổi khóa học, số buổi hoặc ví học viên." onClose={onClose} onSubmit={onSubmit} size="lg" footer={<div className="flex flex-col-reverse gap-2 sm:flex-row sm:items-center sm:justify-between"><button type="button" className="neu-list-item rounded-2xl px-4 py-3 text-sm font-semibold text-stone-600 hover:text-brand-red" onClick={onClose}>Hủy</button><button type="submit" disabled={isSubmitting} className="glass-button-primary inline-flex items-center justify-center gap-2 rounded-2xl px-4 py-3 text-sm font-semibold disabled:cursor-not-allowed disabled:opacity-60"><Plus className="h-4 w-4" />{isSubmitting ? "Đang lưu" : "Lưu phiếu thu"}</button></div>}>
      <div className="grid gap-3 md:grid-cols-2">
        ...
        <label><span className="text-sm font-medium text-stone-600">Người nộp tiền</span><select className="neu-pressed mt-2 w-full rounded-2xl bg-transparent px-4 py-3 text-sm font-medium text-brand-ink outline-none" value={form.payerMode} onChange={(event) => setForm((current) => ({ ...current, payerMode: event.target.value as OtherIncomeReceiptFormState["payerMode"], payerParentPhone: "", payerName: "", payerPhone: "" }))}><option value="PARENT">Chọn phụ huynh</option><option value="MANUAL">Nhập tay</option></select></label>
        {form.payerMode === "PARENT" ? <label className="md:col-span-2"><span className="text-sm font-medium text-stone-600">Phụ huynh</span><select className="neu-pressed mt-2 w-full rounded-2xl bg-transparent px-4 py-3 text-sm font-medium text-brand-ink outline-none" value={form.payerParentPhone} onChange={(event) => { const parent = parents.find((item) => item.phone === event.target.value); setForm((current) => ({ ...current, payerParentPhone: event.target.value, payerName: parent?.name ?? "", payerPhone: parent?.phone ?? "" })) }} required><option value="">Chọn phụ huynh</option>{parents.map((parent) => <option key={parent.phone} value={parent.phone}>{parent.name} - {parent.phone}</option>)}</select></label> : <><FinanceInput label="Họ tên người nộp" value={form.payerName} onChange={(value) => setForm((current) => ({ ...current, payerName: value }))} required /><FinanceInput label="Số điện thoại người nộp" value={form.payerPhone} onChange={(value) => setForm((current) => ({ ...current, payerPhone: value }))} /></>}
